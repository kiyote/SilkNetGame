...
public sealed unsafe class TtfFont : IDisposable {
	...
	public TtfFont(
		...
		fixed( byte* ptr = _fontData ) {
			if( StbTrueType.stbtt_InitFont( _fontInfo, ptr, 0 ) == 0 ) {
				throw new Exception( "Failed to initialize font data." );
			}
		}
	...
	public void RenderTo(
		...
		fixed( byte* ptr = textureData ) {
			_gl.TexSubImage2D(
				...
				0,                      // Mipmap level
				x,          // X destination offset inside the texture
				y,          // Y destination offset inside the texture
				(uint)width,            // Width of the sub-image
				(uint)height,           // Height of the sub-image
				PixelFormat.Rgba,       // Pixel format of your CPU byte array
	...
	public byte[] GenerateTextureData(
		...
		var renderedGlyphs = new List<(GlyphMetrics Metrics, Vector2 FinalPos)>();
		...
		for( int i = 0; i < glyphCount; i++ ) {
			...
			if( glyph.Width > 0 && glyph.Height > 0 ) {
				renderedGlyphs.Add( (glyph, new Vector2( xPos, yPos )) );
				minX = Math.Min( minX, xPos );
				minY = Math.Min( minY, yPos );
				maxX = Math.Max( maxX, xPos + glyph.Width );
				maxY = Math.Max( maxY, yPos + glyph.Height );
		...
		byte[] compositePixels = new byte[outputWidth * outputHeight * 4];
